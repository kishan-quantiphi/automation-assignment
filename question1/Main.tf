...
module "private_ec2" {
  ...
  user_data = <<EOF
    ...
    RDS_MYSQL_ENDPOINT="${element(split(":", aws_db_instance.kishan_RDS.endpoint), 0)}";
    RDS_MYSQL_USER="kishan";
    RDS_MYSQL_PASS="pass@123";
    RDS_MYSQL_BASE="kishan-db";
    mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE -e 'quit';
    if [[ $? -eq 0 ]]; then
      echo "MySQL connection: OK" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='CREATE TABLE kishan1( id INT)';
      echo "MySQL table create" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='INSERT into kishan1 (id) VALUES (1)';
      echo "MySQL insert" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='SELECT * from kishan1' >> conn;
...
module "kishan_RDS" {
  ...
  subnet_group_name = "kishan_subnet_group"
